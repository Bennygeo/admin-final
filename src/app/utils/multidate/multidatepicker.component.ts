...
export class MultidatepickerComponent implements OnInit {
  ...
  @Output() datesSelectedChange: any;
  ...
  constructor(
    ...
    this.datesSelectedChange = new EventEmitter<any>();
  }
  ...
  changeMonth(event, disable, add) {
    ...
    if (!prevented) {
      let oldMonth = moment(this.month.add(90, 'day'));
      this.month = monthTo;
  ...
  onDoneAction() {
    this.datesSelectedChange.emit(this.projectScope);
  }
}
